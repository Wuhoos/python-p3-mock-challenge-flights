class Customer:
    ...
    def has_booked_flight(self, flight):
        if not (flight in self.flight_list):
            return False
        else:
            return True
...
class Flight:
    ...
    def average_price(self):
        if ([booking for booking in Booking.all if booking.flight is self]) == 0:
        # if (len(self.booking_list) == 0) or len(Booking.all) == 0:
            return 0.0
        else:
...
class Booking:    
    ...
    def __init__(self, customer, flight, price):
        ...
        if not (customer in self.flight.customer_list):
            self.flight.customer_list.append(customer)

        self.customer.book_list.append(self)

        if not (flight in self.customer.flight_list):
            self.customer.flight_list.append(flight)
